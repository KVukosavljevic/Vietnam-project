...
def main(
    ...
    if datatype == 'shimmer':
        ...
        for file in file_names:
            ...
            # Obtain features
            try:
                features = get_features_ecg(data, columns)
        ...
        for file in file_names[:3]:
            ...
            try:
                ...
                if label == 0 and visit == 1:
                    tr_low_ppg.extend(features)
                elif label == 1 and visit == 1:
                    tr_high_ppg.extend(features)
                elif label == 0 and visit == 2:
                    ts_low_ppg.extend(features)
                elif label == 1 and visit == 2:
                    ts_high_ppg.extend(features)
                else:
                    print(f"Not categorised for label {label} and visit {visit}.")
            except:
                print(f"Not able to obtain features for file: {file}")

        tr_low_ppg = np.array(tr_low_ppg)
        tr_high_ppg = np.array(tr_high_ppg)

        ts_low_ppg = np.array(ts_low_ppg)
        ts_high_ppg = np.array(ts_high_ppg)
        
        # Data shapes
        print(f"Training data shapes: low risk {tr_low_ppg.shape} and high risk {tr_high_ppg.shape}.")
        print(f"Test data shapes: low risk {ts_low_ppg.shape} and high risk {ts_high_ppg.shape}.")

        # Saving the data
        np.save('tr_low_ppg', tr_low_ppg)
        np.save('tr_high_ppg', tr_high_ppg)

        np.save('ts_low_ppg', ts_low_ppg)
        np.save('ts_high_ppg', ts_high_ppg)
